...
def f(z,t):
    return E0*np.exp(-0.5*((z)/sigma)**2)*np.cos(k*z)+E0*np.exp(-0.5*((-z-c*t)/sigma)**2)*np.cos(-k*z-(w+alpha*t)*t)
    
#def I(z,t):
#    return (E0*np.exp(-0.5*((z)/sigma)**2)*np.cos(2*np.pi*(z)/lam)+E0*np.exp(-0.5*((-z-c*t)/sigma)**2)*np.cos(2*np.pi*(-z-2*c*t)/lam))**2
def I(z,t):
    return (E0*np.exp(-0.5*((z)/sigma)**2)*np.cos(k*z)+E0*np.exp(-0.5*((-z-c*t)/sigma)**2)*np.cos(-k*z-(w+alpha*t)*t))**2
