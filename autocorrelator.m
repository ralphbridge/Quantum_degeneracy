...
syms n_air(x) n_bk7(x)

n_air=1+0.05792105/(238.0185-(1e-12)*(x/(2*pi*c))^2)+0.00167917/(57.362-(1e-12)*(x/(2*pi*c))^2);
n_bk7=sqrt(1+1.03961212*(1e12)*(2*pi*c/x)^2/((1e12)*(2*pi*c/x)^2-0.00600069867)+0.231792344*(1e12)*(2*pi*c/x)^2/((1e12)*(2*pi*c/x)^2-0.0200179144)+1.01046945*(1e12)*(2*pi*c/x)^2/((1e12)*(2*pi*c/x)^2-103.560653));

n0_air=n_air(w0);
np0_air_f=diff(n_air);
np0_air=np0_air_f(w0);
npp0_air_f=diff(n_air,2)
npp0_air=npp0_air_f(w0);
nppp0_air_f=diff(n_air,3);
nppp0_air=nppp0_air_f(w0);
...
n0_bk7=n_bk7(w0);
np0_bk7_f=diff(n_bk7);
np0_bk7=np0_bk7_f(w0);
npp0_bk7_f=diff(n_bk7,2);
npp0_bk7=npp0_bk7_f(w0);
nppp0_bk7_f=diff(n_bk7,3);
nppp0_bk7=nppp0_nbk7_f(w0);

k0_bk7=double(n0_bk7*w0/c);
kp0_bk7=double((n0_bk7+w0*np0_bk7)/c);
kpp0_bk7=double((2*np0_bk7+w0*npp0_bk7)/c);
kppp0_bk7=double((3*npp0_bk7+w0*nppp0_bk7)/c);

function res=InverseFourier(Fw,w,t)
    res=zeros(legth(w))
    for j=1:length(t)
        for i=1:length(w)
            ft(j)=ft(j)+Fw(i)*np.exp(1j*w(i)*t(j))+conj(Fw(i))*exp(-1j*w(i)*t(j));
        end
    end
    res=real(res)
end
...
function res=S_l(t,E0)
    S=zeros(len(t));
    for i=1:length(t)            
        S(i)=trapz((E0+E0shift(E0,j)).^2,t);
    end
    res=S;
end
